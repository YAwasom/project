...
Modifed for Mongo Atlas support
"""
...
class DocumentDbIndexTool(IndexToolConstants):
    ...
    def _get_db_connection(
        ...
        logging.debug("Connecting to instance to instance")
        ...
        if password is not None:
            ...
            mongodb_client[auth_db].authenticate(name=username, password=password)
            logging.info("Successfully authenticated to database: %s", auth_db)

        logging.info("Successfully connected to instance")
    ...
    def _get_compatible_metadata(self, metadata, compatibility_issues):
        ...
        for db_name in compatibility_issues:
            ...
            for collection_name in compatibility_issues[db_name]:
                if (
                    self.UNSUPPORTED_COLLECTION_OPTIONS_KEY
                    in compatibility_issues[db_name][collection_name]
                ):
                ...
                if (
                    ...
                    del compatible_metadata[db_name][collection_name]
                    continue
                for index_name in compatibility_issues[db_name][collection_name]:
                    del compatible_metadata[db_name][collection_name][self.INDEXES][
                        index_name
                    ]
    ...
    def _get_metadata_from_file(self, filepath):
        ...
        with open(filepath, "rt") as metadata_file:
            ...
            if indexes is None:
                raise Exception(
                    "Malformed metadata document {} has no indexes.".format(filepath)
                )
            ...
            for index in indexes:
                ...
                if file_metadata[self.OPTIONS]:
                    collection_metadata[self.OPTIONS] = file_metadata[self.OPTIONS]
        ...
        try:
            ...
            for database_doc in database_info["databases"]:
                ...
                for collection_name in connection[
                    ...
                    collection_metadata[self.OPTIONS] = connection[database_name][
                        collection_name
                    ].options()
                    ...
                    if dry_run is True:
                        ...
                        with open(
                            collection_metadata_filepath, "wt"
                        ) as collection_metadata_file:
                            collection_metadata_file.write(
                                dumps(collection_metadata, separators=(",", ":"))
                            )

            logging.info(
                "Completed writing index metadata to local folder: %s", output_dir
            )
    ...
    def get_metadata(self, start_path):
        ...
        try:
            ...
            if metadata_files == []:
                logging.error(
                    "No metadata files found beneath directory: %s", start_path
                )
                sys.exit()
            ...
            for filepath in metadata_files:
                (
                    ...
                    collection_name,
                    collection_metadata,
                ) = self._get_metadata_from_file(filepath)
        ...
        for db_name in metadata:
            ...
            for collection_name in metadata[db_name]:
                ...
                if len(collection_namespace) > DocumentDbLimits.NAMESPACE_MAX_LENGTH:
                    ...
                    compatibility_issues[db_name][collection_name][
                        self.EXCEEDED_LIMITS
                    ][message] = collection_namespace

                if self.OPTIONS in collection_metadata:
                    for option_key in collection_metadata[self.OPTIONS]:
                        if (
                            option_key
                            in DocumentDbUnsupportedFeatures.UNSUPPORTED_COLLECTION_OPTIONS
                            and collection_metadata[self.OPTIONS][option_key] is True
                        ):
                            if (
                                self.UNSUPPORTED_COLLECTION_OPTIONS_KEY
                                not in compatibility_issues[db_name][collection_name]
                            ):
                                compatibility_issues[db_name][collection_name][
                                    self.UNSUPPORTED_COLLECTION_OPTIONS_KEY
                                ] = []
                ...
                for index_name in collection_metadata[self.INDEXES]:
                    ...
                    if (
                        len(fully_qualified_index_name)
                    ...
                    for key_name in index:
                        ...
                        if (
                            key_name
                            in DocumentDbUnsupportedFeatures.UNSUPPORTED_INDEX_OPTIONS
                        ):
                            if (
                                self.UNSUPPORTED_INDEX_OPTIONS_KEY
                                not in compatibility_issues[db_name][collection_name][
                                    index_name
                                ]
                            ):
        ...
        for db_name in metadata:
            for collection_name in metadata[db_name]:
                for index_name in metadata[db_name][collection_name][self.INDEXES]:
                    ...
                    index_keys = metadata[db_name][collection_name][self.INDEXES][
                        index_name
                    ][self.INDEX_KEY]
                    keys_to_create = []
                    index_options = {}
                    ...
                    for k in metadata[db_name][collection_name][self.INDEXES][
                        index_name
                    ]:
                        if k != self.INDEX_KEY and k != self.INDEX_VERSION:
                            # this key is an additional index option
                            index_options[k] = metadata[db_name][collection_name][
                                self.INDEXES
                            ][index_name][k]
                    ...
                    if self.args.dry_run is True:
                        ...
                        logging.debug(
                            "Adding index %s -> %s", keys_to_create, index_options
                        )
    ...
    def run(self):
        ...
        if self.args.dump_indexes is True or self.args.restore_indexes is True:
            try:
                connection_string = self.args.atlas_connection

                # Use document db vs atlas
                if (
                    self.args.atlas_connection == "localhost"
                    or not self.args.atlas_connection
                ):
                    connection_string = ""
        ...
        if self.args.dump_indexes is True:
            self._dump_indexes_from_server(connection, self.args.dir, self.args.dry_run)
            sys.exit()
...
def main():
    ...
    parser.add_argument(
        "--dir", required=True, type=str, help="dump to or restore from DIR"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="localhost",
        help="connect to host Atlas (default: localhost)",
    )
    ...
    parser.add_argument(
        "--tls", required=False, action="store_true", help="connect using TLS"
    )
