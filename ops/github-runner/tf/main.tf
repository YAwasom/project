...
resource "aws_sqs_queue" "queued_builds" {
  ...
  kms_master_key_id           = aws_kms_key.sqs_kms.key_id
  tags                        = var.tags
}

resource "aws_sqs_queue_policy" "queued_builds" {
  queue_url = aws_sqs_queue.queued_builds.id

  policy = templatefile("${path.module}/policies/queued-builds-sqs-policy.json", {
    sqs_arn        = aws_sqs_queue.queued_builds.arn
...
module "webhook" {
  ...
  encryption_sqs = {
    kms_key_id  = aws_kms_key.sqs_kms.key_id
    kms_key_arn = aws_kms_key.sqs_kms.arn
  }
...
module "runners" {
  ...
  encryption_gold_ami = {
    kms_key_id  = var.gold_ami_kms_key_id
    kms_key_arn = var.gold_ami_kms_key_arn
  }
...
module "runner_binaries" {
  ...
  distribution_bucket_name = "${var.environment}-github-runner-lambda-dist-${random_string.random.result}"
