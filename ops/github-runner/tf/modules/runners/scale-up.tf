...
resource "aws_kms_grant" "scale_up_sqs" {
  count             = 1
  name              = "${var.environment}-scale-up"
  key_id            = var.encryption_sqs.kms_key_id
  grantee_principal = aws_iam_role.scale_up.arn
  operations        = ["Decrypt"]
...
resource "aws_iam_role_policy" "scale_up" {
  ...
  policy = templatefile("${path.module}/policies/lambda-scale-up.json", {
    ...
    gold_ami_kms_key_arn     = var.encryption_gold_ami.kms_key_arn
  })
}

resource "aws_iam_role_policy_attachment" "scale_up_xray" {
  role       = aws_iam_role.scale_up.name
